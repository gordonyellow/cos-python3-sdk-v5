...
from qcloud_cos.xml2dict import Xml2Dict
...
class CosConfig(object):
    ...
    def uri(self, bucket, path=None, scheme=None, region=None):
        ...
        print('bucket=%s,region=%s,to_unicode(bucket)=%s' % (bucket,region, to_unicode(bucket)))
        if scheme is None:
            scheme = self._scheme
        if region is None:
            region = self._region
...
class CosS3Client(object):
    ...
    def send_request(self, method, url, timeout=30, **kwargs):
        """封装request库发起http请求"""
        logger.info('type(url)=%s', type(url))
